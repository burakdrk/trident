...
struct FFZService: ThirdPartyEmoteService {
    ...
    func channelEmotes(for channelID: String) async throws -> [Emote] {
        ...
        let response: FFZChannelEmoteResponse = try await URLSession.perform(.init(url: urlString, timeoutInterval: requestTimeout))
    ...
    func globalEmotes() async throws -> [Emote] {
        ...
        let response: FFZGlobalEmoteResponse = try await URLSession.perform(.init(url: urlString, timeoutInterval: requestTimeout))
