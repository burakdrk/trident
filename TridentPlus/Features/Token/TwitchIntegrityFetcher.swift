...
class TwitchIntegrityFetcher: NSObject, WKNavigationDelegate {
    private static let endpoint = "https://www.twitch.tv/settings"
    private static let scriptSrc = "https://k.twitchcdn.net/149e9513-01fa-4fb0-aad4-566afd725d1b/2d206a39-8ed7-437e-a3be-862e0f06eea3/p.js"
    ...
    init(onTokenReceived: @escaping TokenReceiveHandler) {
        self.webView = WKWebView(frame: .zero, configuration: TwitchIntegrityFetcher.makeConfig())
    ...
    func start() {
        let request = URLRequest(url: URL(string: TwitchIntegrityFetcher.endpoint)!)
